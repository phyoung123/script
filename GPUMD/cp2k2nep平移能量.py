...
#
#
#   先利用 Get_energy 函数提取到了所有结构的能量平均值，然后用各个能量减去这个平均值即完成能量平移

#   需要注意的是这个脚本只适用于每个结构的原子数相同
...
def Get_energy(folder, fnames=None):
    ...
    with open(frc_file) as ffc:

        while True:
            frc_header = ffc.readline()

            if not frc_header:
                break

            num_atoms = int(frc_header.strip().split()[0])

            frc_info_line = ffc.readline()
...
mean_energy = Get_energy('.', fnames=None)
...
def CP2K2XYZ(folder, fnames=None, output_file=None):
    ...
    with open(pos_file, 'r') as pf, open(frc_file, 'r') as ff, \
        ...
        while True:
            ...
            energy = float(frc_info_line.strip().split("E =")[-1]) * 27.211386245988
            energy -= mean_energy    # I first calculate the mean energy using Get_energy function


            # Read and process the cell information
            #cell_line = cf.readlines()
            # Only read the Ax, Ay, Az, Bx, By, Bz, Cx, Cy, Cz columns
            #lattice = " ".join(cell_line[:])
            ...
            for _ in range(num_atoms):
                ...
                force_x = float(frc_line[1]) * 51.42206747632590000   # force unit hartree bohr^-1
                force_y = float(frc_line[2]) * 51.42206747632590000   # hartree bohr^-1  to eV A^-1   51.4220674763
